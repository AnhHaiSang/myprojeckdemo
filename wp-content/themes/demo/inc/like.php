...
class jws_theme_jwsLove {
	...
	function add_love($unit = '',$show_icon = true) {
		...
		$output = $this->love_post($post->ID);
  		...
  		$title = esc_html__('Love this', 'fatcy');
		if( isset($_COOKIE['jws_love_'. $post->ID]) ){
			$class = 'jws-btn jws-love loved';
			$title = esc_html__('You already love this!', 'fatcy');
		}

		return '<a href="#" class="'. $class .'" id="jws-love-'. $post->ID .'" title="'. $title .'">
		
		<span class="count">'.$output .'</span><span class="like_text">'.$unit.'</span>
		</a>';
	}
