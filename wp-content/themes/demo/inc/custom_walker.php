...
class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu
{
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $current_object_id = 0)
    {
        ...
        if (!empty($item->_invalid)) {
            ...
            $title = sprintf(esc_html__('%s (Invalid)', 'fatcy'), $item->title);
        } elseif (isset($item->post_status) && 'draft' == $item->post_status) {
            $classes[] = 'pending';
            /* translators: %s: title of menu item in draft status */
            $title = sprintf(esc_html__('%s (Pending)', 'fatcy'), $item->title);
        }
            ...
            <dt class="menu-item-handle">
                ...
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'fatcy'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                        echo wp_nonce_url(
                            add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr
                                    title="<?php esc_attr_e('Move down', 'fatcy'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>"
                       title="<?php esc_attr_e('Edit Menu Item', 'fatcy'); ?>" href="<?php
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Navigation Label', 'fatcy'); ?><br/>
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Title Attribute', 'fatcy'); ?><br/>
            ...
            <p class="field-link-target description">
                <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                    ...
                    <?php esc_html_e('Open link in a new window/tab', 'fatcy'); ?>
                </label>
            </p>
            <p class="field-css-classes description description-thin">
                <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('CSS Classes (optional)', 'fatcy'); ?><br/>
            ...
            <p class="field-xfn description description-thin">
                <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Link Relationship (XFN)', 'fatcy'); ?><br/>
            ...
            <p class="field-css-brand description description-thin">
                <label for="edit-menu-item-brand-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Brand Label Ex( Hot , New ...)', 'fatcy'); ?><br/>
            ...
            <p class="field-brandbg description description-thin">
                <label for="edit-menu-item-brandbg-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Brand Bg Ex(#000000 , red)', 'fatcy'); ?><br/>
            ...
            <?php if (!$depth) { ?>
                <p class="field-description description description-wide">
                    <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e('Add Shortcode for mega menu', 'fatcy'); ?><br/>
                ...
                <p class="description description-wide jws-design">
                    ...
                    <label for="edit-<?php echo esc_attr($key . '-' . $item_id); ?>">
                        <select id="edit-<?php echo esc_attr($key . '-' . $item_id); ?>"
                            ...
                            <option value="standard" <?php echo wp_kses_post(($value == 'standard') ? ' selected="selected" ' : ''); ?>><?php esc_html_e('Standard Dropdown', 'fatcy'); ?></option>
                            <option value="mega_menu" <?php echo wp_kses_post(($value == 'mega_menu') ? ' selected="selected" ' : ''); ?>><?php esc_html_e('Mega With Set Size', 'fatcy'); ?></option>
                            <option value="mega_menu_full_width" <?php echo wp_kses_post(($value == 'mega_menu_full_width') ? ' selected="selected" ' : ''); ?>><?php esc_html_e('Mega Full Width', 'fatcy'); ?></option>
                        </select>
                    </label>
                </p>
                <?php
            ...
            if (!$depth) {
                ...
                <p class="field-submenu_size description description-thin jws-width">
                    <label for="edit-menu-item-submenu_size-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e('Add Width Mega Menu ', 'fatcy'); ?><br/>
            ...
            */
                ...
                <p class="description-thin jws-position">
                    ...
                    <label for="edit-<?php echo esc_attr($key . '-' . $item_id); ?>">
                        <select id="edit-<?php echo esc_attr($key . '-' . $item_id); ?>"
                                class=" <?php echo esc_attr($key); ?>"
                                name="<?php echo esc_attr($key . "[" . $item_id . "]"); ?>">
                            <option value="center" <?php echo wp_kses_post(($value == 'center') ? ' selected="selected" ' : ''); ?>><?php esc_html_e('Center', 'fatcy'); ?></option>
                            <option value="left" <?php echo wp_kses_post(($value == 'left') ? ' selected="selected" ' : ''); ?>><?php esc_html_e('Left', 'fatcy'); ?></option>
                            <option value="right" <?php echo wp_kses_post(($value == 'right') ? ' selected="selected" ' : ''); ?>><?php esc_html_e('Right', 'fatcy'); ?></option>
                        </select>
                    </label>
                </p>
                <?php
            ...
            ?>
            <p class="field-menu_icon description description-wide">
                <label for="edit-menu-item-menu_icon-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Add Icon Menu ', 'fatcy'); ?><br/>
                    <input type="text" id="edit-menu-item-menu_icon-<?php echo esc_attr($item_id); ?>"
                           class="widefat code edit-menu-item-menu_icon"
                           name="menu-item-menu_icon[<?php echo esc_attr($item_id); ?>]"
                           value="<?php echo esc_attr($item->menu_icon); ?>"/>
                </label>
            </p>
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if ('custom' != $item->type && $original_title !== false) : ?>
                    <p class="link-to-original">
                        <?php printf(esc_html__('Original: %s', 'fatcy'), '<a href="' . esc_attr($item->url) . '">' . esc_html($original_title) . '</a>'); ?>
                    </p>
                <?php endif; ?>
                ...
                echo wp_nonce_url(
                    ...
                    'delete-menu_item_' . esc_attr($item_id)
                ); ?>"><?php esc_html_e('Remove', 'fatcy'); ?></a> <span class="meta-sep"> | </span> <a
                        ...
                        ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'fatcy'); ?></a>
            </div>
